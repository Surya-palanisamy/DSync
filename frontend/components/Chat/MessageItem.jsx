...
const MessageItem = memo(
  ({
    ...
    const getMessageStatus = () => {
      ...
      if (isOptimistic) {
        if (status === "sending") {
          return <Clock size={12} className="status-icon status-sent" />;
      ...
      if (status === "seen") {
        return <CheckCheck size={12} className="status-icon status-read" />;
      }
      if (status === "delivered") {
        return (
      ...
      return <Check size={12} className="status-icon status-sent" />;
    };
    ...
    return (
      <motion.div
        className={`message ${isOwn ? "sent" : "received"} ${isOptimistic ? "optimistic" : ""}`}
          ...
          <div className="message-bubble-container">
            ...
            >
              ...
              <div className="message-meta">
                ...
                {isOwn && (
                  <div className="message-status">{getMessageStatus()}</div>
                )}
                {status === "failed" && (
                  <button className="retry-btn" title="Retry sending">
                    <RefreshCw size={10} />
                  </button>
                )}
              </div>
